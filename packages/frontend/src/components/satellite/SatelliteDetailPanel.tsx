...
import { getRiskLevelColor, RISK_COLORS } from '../../utils/colors';
...
export function SatelliteDetailPanel({ satellite, onClose }: Props) {
    ...
    const isWatched = useWatchlistStore((s) => s.watchedIds.includes(satellite.id));
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                {/* Risk score */}
                <div className="flex items-center gap-3 mb-3 p-2 rounded-lg" style={ { backgroundColor: `${riskColor}15` } }>
                    <div
                    ...
                    <div>
                        { satellite.riskLevel && <LevelBadge level={ satellite.riskLevel } size="md" /> }
                        <div className="font-mono text-[10px] text-text-muted mt-0.5">risk score</div>
                    </div>
                </div>

                {/* Orbital info */}
                <div className="grid grid-cols-2 gap-x-3 gap-y-1 mb-3">
                    <div className="font-mono text-[10px] text-text-muted">Regime</div>
                    <div className="font-mono text-xs text-text-primary">
                        { satellite.orbitRegime ?? '—' }
                    </div>
                    <div className="font-mono text-[10px] text-text-muted">Altitude</div>
                    <div className="font-mono text-xs text-text-primary">
                        { formatAlt(satellite.alt) }
                    </div>
                    <div className="font-mono text-[10px] text-text-muted">Position</div>
                    <div className="font-mono text-xs text-text-primary">
                        { formatCoord(satellite.lat) }&deg;, { formatCoord(satellite.lng) }&deg;
                    </div>
                    <div className="font-mono text-[10px] text-text-muted">Sunlit</div>
                    <div className="font-mono text-xs text-text-primary flex items-center gap-1">
                        { satellite.isSunlit
                            ? <><Sun size={ 10 } className="text-yellow-400" /> Yes</>
                            : <><Moon size={ 10 } className="text-blue-400" /> Shadow</>
                        }
                    </div>
                    <div className="font-mono text-[10px] text-text-muted">SAA</div>
                    <div className="font-mono text-xs text-text-primary flex items-center gap-1">
                        { satellite.isInSAA
                            ? <><AlertTriangle size={ 10 } className="text-risk-high" /> Inside</>
                            : <><Shield size={ 10 } className="text-text-muted" /> Outside</>
                        }
                    </div>
                </div>
                    ...
                    <div className="mb-3">
                        ...
                        <div className="space-y-1">
                            { Object.entries(BREAKDOWN_LABELS).map(([key, label]) => (
                                <BreakdownBar
                                    key={ key }
                                    label={ label }
                                    value={ breakdown.scoring[key as keyof typeof breakdown.scoring] as number }
                        ...
                        { breakdown.scoring.bzMultiplier > 1 && (
                            <div className="font-mono text-[10px] text-text-muted mt-1 text-right">
                                Bz multiplier: &times;{ breakdown.scoring.bzMultiplier }
                            </div>
                        ) }
                    </div>
                ) }
                    ...
                    <div>
                        ...
                        <ul className="space-y-0.5">
                            { satellite.threats.map((t, i) => (
                                ...
                                >
                                    <span className="text-risk-high mt-0.5 shrink-0">&bull;</span>
                                    { t }
                                </li>
                            )) }
                        </ul>
