        ...
        return (
            <GlassPanel title="Watchlist" icon={ <Star size={ 14 } /> }>
                <div className="text-center py-6">
                    <Eye size={ 24 } className="mx-auto text-text-muted mb-2 opacity-40" />
                    <p className="font-mono text-[11px] text-text-muted">
                        No satellites watched
                    </p>
                    <p className="font-mono text-[10px] text-text-muted mt-1">
                        Click a satellite on the globe or list,<br />
                ...
                { watched.map((sat) => (
                    ...
                    >
                        <div
                            ...
                            style={ { backgroundColor: getRiskLevelColor(sat.riskLevel) } }
                        />
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="font-mono text-[9px] text-text-muted">
                                { sat.orbitRegime } &middot; { Math.round(sat.alt) }km
                        ...
                        <button
                            onClick={ (e) => { e.stopPropagation(); remove(sat.id); } }
                        ...
                        .map((sat) => (
                            <div key={ sat.id } className="mb-1">
                                ...
                                { sat.threats!.slice(0, 2).map((t, i) => (
                                    <div key={ i } className="font-mono text-[10px] text-risk-high pl-2">
                                        { t }
                                    </div>
                                )) }
                            </div>
